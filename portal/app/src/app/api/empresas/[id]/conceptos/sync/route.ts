/**
 * POST /api/empresas/[id]/conceptos/sync
 * Sincroniza productos de factura.com → billing_conceptos para una empresa.
 */
import type { NextRequest } from "next/server";
import { createClient } from "@/lib/supabase/server";
import { requireAuth, apiSuccess, apiError, getFacturaComCredentials } from "@/lib/api/helpers";
import { syncProductosFacturaCom } from "@/lib/facturacom";

export async function POST(
  _req: NextRequest,
  ctx: RouteContext<"/api/empresas/[id]/conceptos/sync">
) {
  ...
  try {
    ...
    const message =
      err instanceof Error ? err.message : "Error al sincronizar productos";
